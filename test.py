...
def instance_state(token, app, guid, instance_impacted):
    ...
    try:
        ...
        print(f"Number of instances in '{app}' is  - '{number_of_instances}'", flush=True)
    except:
        pass
    try:
        instance_status = response.json()['resources'][instance_impacted]['state']
        print(f"instance status is - {instance_status}", flush=True)
...
if __name__ == '__main__':
    ...
    if tenant_name == "":
        print("No Tenant selected for chaos action.chaos action wil be performed only on MTMS", flush=True)
        ...
        try:
            ...
            print("No worker selected", flush=True)
            pass
    try:
        app_array = app_list + worker_list
    except:
